...
handle_job(SDateTime, #cron{name = _Name,
               						function = {M, F, A},
               						schedule = Schedule}) ->
  case match_times(SDateTime, Schedule) of
  	false -> ok;
  	true -> io:spawn(M, F, [A])
  end.


match_times(SDateTime, JSchedule) ->
  {{_SYYYY, SMM, SDD}, {SHour, SMinute, _SSecond}}  = SDateTime,
  {JMinute, JHour, JDayOfMonth, JMonth, JDayOfWeek} = JSchedule,
  MatchVerb =   [{SMinute, JMinute}, {SHour, JHour},
...
match_values({_Value, '_'}) ->
    %io:format("~n~nMatched _, '_'; ~p~n", [_Value]),
    true;
match_values({Value, Value}) ->
    %io:format("~n~nMatched Value: ~p, ~p~n", [Value,Value]),
    true;
match_values({Value, [$/|Interval]}) ->
    %io:format("~n~nMatched ~p/~p~n", [Value, Interval]),
    case Value rem list_to_integer(Interval) of
      0 -> true;
      _ -> false
		end;
match_values({Value, ValuesList}) when is_list(ValuesList) ->
      %io:format("~n~nMatched Val, [Value, Value...]~n"),
      lists:member(Value, ValuesList);
match_values(_Y) ->
      %io:format("~n~nMatched _;  ~p~n", [_Y]),
      false.
